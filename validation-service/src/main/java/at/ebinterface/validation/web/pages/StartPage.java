...
public class StartPage extends BasePage {
	...
	public StartPage() {
		...
		//Add the input form
		InputForm inputForm = new InputForm("inputForm");
		add(inputForm);
			
		
		//Add the form for showing the supported rules
		ShowRulesForm showRulesForm = new ShowRulesForm("showRulesForm");
		add(showRulesForm);
	...
	private class ShowRulesForm extends Form {
		...
		public ShowRulesForm(String id) {
			super(id);
			...
			List ruleValues = Rules.getRuleslist();		
	...
	private class InputForm extends Form {
		...
		public InputForm(String id) {
			...
			SubmitLink resetButton = new SubmitLink("resetButton") {
				@Override
				public void onSubmit() {
		 ...
		 * Process the input
		 */		
		protected void submit(ActionType selectedAction) {
			...
			if (selectedAction == ActionType.SCHEMA_AND_SCHEMATRON_VALIDATION) {					
				...
				if (validationResult.getDeterminedEbInterfaceVersion() == EbInterfaceVersion.E4P0) {
					...
					Result r = validator.validateXMLInstanceAgainstSchematron(uploadedData, schematronRuleFile);
					validationResult.setResult(r);	
				}
				//Wrong ebInterface version
				else {
					error("Schematronregeln können nur auf ebInterface 4.0 Instanzen angewendet werden. Erkannte ebInterface Version ist jedoch: " + Utils.INSTANCE.getPrettyPrintedEbInterfaceVersion(validationResult.getDeterminedEbInterfaceVersion()));
					onError();
					return;
				}				
			}
			//Visualization?
				...
				String s = validator.transformInput(uploadedData, validationResult.getDeterminedEbInterfaceVersion());				
		...
		/**
		 * Returns the inputstream as string
		 * @param stream
		 * @return
		 * @throws IOException 
		 */
