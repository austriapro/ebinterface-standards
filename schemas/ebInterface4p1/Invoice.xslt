...
hr{
	...
	<xsl:template match="//eb:Invoice/eb:InvoiceDate">
		<tr>
			...
			<td class="nopadding">			
				<xsl:variable name="invoiceDate" select="."/>	
				<xsl:value-of select="concat(substring(string($invoiceDate), 9,2),'.',substring(string($invoiceDate), 6,2), '.', substring(string($invoiceDate), 1,4))"/>
			</td>
		</tr>
	</xsl:template>
